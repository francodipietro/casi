...
static int merge_branch(casi_entry_list *into, casi_entry_list *from)
{
    size_t i;
    int rc;
    ...
    for (i = 0; i < from->len; i++) {
        ...
        if (casi_sync_compare(incoming, have) == CASI_SYNC_LOCAL_AHEAD) {
            casi_entry moved = *incoming;
